...
void aplay_tmedia_format_close(AVFormatContext *avformat) {
    if (avformat) {
        avformat_close_input(&(avformat));
    }
}

AVFormatContext *aplay_tmedia_format_open(TEST_APLAY_CTX_S *params) {
    RK_S32 ret = 0;
    RK_S32 err = 0;
    RK_S32 retryNum = 5;
...
__FAILED:
    ...
    aplay_tmedia_format_close(avformat);

    return RK_NULL;
}

static RK_S32 aplay_tmedia_free(void *opaque) {
...
static RK_U32 aplay_find_audio_codec_id(char *format) {
    ...
    } else if (strstr(format, "tmd")) {
        return RK_AUDIO_ID_TMD;
...
RK_S32 test_init_aplay(TEST_APLAY_CTX_S *ctx) {
    ...
    avfmt = aplay_tmedia_format_open(ctx);
    if (!avfmt) {
        RK_LOGE("failed to tmedia format open file: %s", ctx->srcFilePath);
        return RK_FAILURE;
    }
    ctx->avformat = avfmt;
...
RK_S32 deinit_mpi_aplay(TEST_APLAY_CTX_S *ctx) {
    ...
    aplay_tmedia_format_close(ctx->avformat);
    return RK_SUCCESS;
}
...
__RETRY:
        ...
        if (pktEos) {
            ...
            extConfig.pFreeCB =  aplay_tmedia_free;
...
int main(int argc, const char **argv) {
    ...
    struct argparse_option options[] = {
        ...
        OPT_STRING('C', "codec", &(ctx->chCodecId),
                    "codec, e.g.(mp3, tmd, mp2). <required>", NULL, 0, 0),
