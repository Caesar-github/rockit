...
static RK_S32 mpi_vo_deinit_sample(const MPI_CTX_S *ctx) {
    ...
    if (ctx->stVoCfg.enUIMode == VIDEO_UI_MODE_MERGE) {
        RK_MPI_VO_ReleaseGfxFrameBuffer(VoLayer, 127);
        RK_MPI_VO_ReleaseGfxFrameBuffer(VoLayer, 126);
        if (ctx->stVoCfg.bDoubleScreen) {
            RK_MPI_VO_ReleaseGfxFrameBuffer(VoLayer_second, 127);
            RK_MPI_VO_ReleaseGfxFrameBuffer(VoLayer_second, 126);
        }
    }
...
__RETRY:
    ...
    for (RK_S32 i = 0; i < pstAvfc->nb_streams; i++) {
        ...
        if (RK_NULL != stream && stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ctx->stParserCfg.enCodecId = (RK_CODEC_ID_E)TEST_COMM_CodecIDFfmpegToRK(stream->codecpar->codec_id);
            ...
            ctx->stParserCfg.pExtraData = stream->codecpar->extradata;
            ctx->stParserCfg.u32ExtraDataSize = stream->codecpar->extradata_size;
            RK_LOGD("found video stream width %d height %d extradata size %d",
                ctx->stParserCfg.u32SrcWidth, ctx->stParserCfg.u32SrcHeight, ctx->stParserCfg.u32ExtraDataSize);
            bFindStream = RK_TRUE;
            break;
        }
    }
...
static RK_S32 create_rgn(MPI_CTX_S *ctx) {
    ...
    ctx->stRgnCfg.stRgnAttr.unAttr.stOverlay.u32ClutNum = 0;
...
void* vdec_send_stream_thread(void *pArgs) {
    ...
    while (!ctx->bThreadExit) {
        ...
        if (bFirstFrame && ctx->stParserCfg.u32ExtraDataSize > 0) {
            avPacket->data = reinterpret_cast<uint8_t *>(ctx->stParserCfg.pExtraData);
            avPacket->size = ctx->stParserCfg.u32ExtraDataSize;
            avPacket->stream_index = ctx->stParserCfg.u32StreamIndex;
            avPacket->flags = AV_PKT_FLAG_KEY;
            bFirstFrame = RK_FALSE;
        } else {
            s32Ret = av_read_frame(ctx->stParserCfg.pstAvfc, avPacket);
            if (s32Ret == AVERROR_EOF || s32Ret == AVERROR_EXIT) {
                mpi_tmedia_free(avPacket);
                if (ctx->s32LoopCount > 0) {
                    break;
                }

                RK_LOGD("eos %d ret", ctx->u32ChnIndex);
       ...
       if (ctx->stParserCfg.u32StreamIndex == avPacket->stream_index) {
            ...
            if (bEos) {
                RK_LOGD("entern send %d", ctx->u32ChnIndex);
                bEos = RK_FALSE;
            }

            stMbExtConfig.pFreeCB = mpi_tmedia_free;
            ...
            vStream = ctx->stParserCfg.pstAvfc->streams[avPacket->stream_index];
...
RK_S32 unit_mpi_vplay_test(MPI_CTX_S *ctx) {
    ...
    pthread_t vencThread = 0;
    RK_U32 u32VdecSendThreadNum = 0;
    RK_U32 u32VdecGetThreadNum = 0;
    RK_U32 u32Ch = 0;

    memset(vdecSendThread, 0, sizeof(vdecSendThread));
    memset(vdecGetThread, 0, sizeof(vdecGetThread));
    memset(voThread, 0, sizeof(voThread));
...
void init_cfg(MPI_CTX_S *ctx) {
    ...
    ctx->stWbcCfg.stWbcAttr.u32FrameRate = 30;
